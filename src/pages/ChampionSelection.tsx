...
function ChampionSelection({
  ...
  const previewedChampion =
    preselectedChampion ?? (activePhase === "warmup" ? localWarmupHoverChampion : undefined);
  ...
  const activePickerNames = activePickGroup
    .map((player) => {
      return getPublicDisplayName(player.displayName, `#${player.publicId ?? "?"}`);
    })
    .filter(Boolean);
  const bubbleChampionWallpapers =
    activePhase === "ready"
      ? [selectedChampionWallpaper].filter((wallpaper): wallpaper is string => Boolean(wallpaper))
      : activePickerChampions
      ...
      <section className="champion-selection-layout">
        {teams.map((team, teamIndex) => (
          <aside
            className={
              teamIndex === 0
                ? "champion-selection-team champion-selection-team-dark"
                : "champion-selection-team champion-selection-team-light"
            }
            ...
            <div className="champion-selection-team-list">
              {(team.players ?? []).map((player) => {
                ...
                const playerHoveredChampion =
                  player.publicId === currentPlayerPublicId && activePhase === "warmup"
                    ? previewedChampion
                    : getPlayerHoveredChampion(match, player.publicId);
            ...
            <div className="champion-selection-picked">
              <span>
                {activePhase === "ready"
                  ...
                  : activePickerNames.join(", ") || t("champion-select-opponent")}
              </span>
