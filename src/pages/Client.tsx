...
function Client({
  ...
  const [gameStartedMatch, setGameStartedMatch] = useState<_8083ApiMatchResponse>();
  ...
  const matchFoundMaxAcceptCount =
    ...
    0;
  const gameStartedDebugPlayers = useMemo(() => {
    if (!gameStartedMatch) {
      return [];
    }

    return (gameStartedMatch.lobbies ?? []).flatMap((lobby, lobbyIndex) => {
      return (lobby.players ?? []).map((player) => {
        const selection = gameStartedMatch.championSelections?.find((championSelection) => {
          return championSelection.playerPublicId === player.publicId;
        });

        return {
          champion: selection?.champion,
          key: `${lobby.lobbyId ?? lobbyIndex}-${player.publicId ?? player.displayName ?? "player"}`,
  ...
  async function refreshLobbyFriendProfiles() {
    ...
    if (result.error) {
      setLobbyError(t("friend-api-error"));
      return;
    }
  ...
  async function handleCreateLobby() {
    ...
    if (result.error || !result.data?.id) {
      setLobbyError(t("lobby-create-error"));
      return;
    }
  ...
  async function handleLobbySearch() {
    ...
    if (result.error) {
      setLobbyError(t("lobby-search-error"));
      return;
    }

    const matchSearchResult = await startSearch({
      baseUrl: MATCHMAKING_API_BASE_URL,
      body: {
        lobbyId: activeLobby.id,
        mode: "RANKED",
  ...
  async function handleMatchDecision(decision: MatchDecision) {
    ...
    if (!nextMatch && decision === "accept") {
      setLobbyError(t("match-decision-error"));
      return;
    }
  ...
  async function handleLobbyFriendDrop(friend: FriendProfile) {
    ...
    if (result.error) {
      setLobbyError(t("lobby-invite-error"));
      return;
    }
  ...
  async function handleMakeLobbyHost(member: LobbyMember) {
    ...
    if (result.error || !result.data) {
      setLobbyError(t("lobby-host-transfer-error"));
      return;
    }
  ...
  async function handleKickMember(member: LobbyMember) {
    ...
    if (result.error || !result.data) {
      setLobbyError(t("lobby-kick-error"));
      return;
    }
  ...
  async function handleJoinFriendParty(lobby: LobbySnapshot) {
    ...
    if (result.error || !result.data) {
      setLobbyError(t("lobby-join-error"));
      return;
    }
  ...
  return (
    <>
      ...
      <section className="dashboard-panel" aria-label="Dashboard">
        ...
        {lobbyError ? <p className="message error">{lobbyError}</p> : null}
        {gameInProgress ? (
          <div className="client-game-running-message" role="status">
            <strong>{t("client-game-running")}</strong>
            {gameStartedMatch ? (
              <div className="client-game-debug" aria-label={t("client-game-debug-title")}>
                <span>{t("client-game-debug-title")}</span>
                <small>
                  {gameStartedMatch.matchId} · {gameStartedMatch.mode ?? "Ranked"}
                </small>
                <div className="client-game-debug-list">
                  {gameStartedDebugPlayers.map(({ champion, key, lobbyId, player }) => {
                    const playerName = getPublicDisplayName(
                      player.displayName,
                      `#${player.publicId ?? "?"}`,
                    );

                    return (
                      <div className="client-game-debug-row" key={key}>
                        <span>{playerName}</span>
                        <small>
                          #{player.publicId ?? "?"} · {lobbyId ?? "-"}
                        </small>
