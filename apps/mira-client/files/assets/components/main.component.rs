...
use mira_moba_systems::MiraHudState;
use serde::Serialize;
...
impl Plugin for MainComponentPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<MainHudStoreUpdateTimer>()
            ...
            .add_systems(
                PostUpdate,
                sync_hud_health_fill.after(apply_calc_styles_system),
            );
    }
}
...
/// Description:
/// Applies health fill width directly to the existing HUD fill node.
///
/// Params:
/// - `hud_state`: Gameplay HUD state that provides the local health percentage.
/// - `fill_nodes`: Extended UI nodes with CSS ids that can be updated in place.
fn sync_hud_health_fill(hud_state: Res<MiraHudState>, mut fill_nodes: Query<(&CssID, &mut Node)>) {
    let next_width = Val::Percent(hud_state.health_percent.clamp(0.0, 100.0));

    for (css_id, mut node) in &mut fill_nodes {
        if css_id.0 != HUD_HEALTH_FILL_ID || node.width == next_width {
            continue;
        }
